...
namespace InventarioGEITests
{
    ...
    {
        public ReportesController controller;
        public Context _dbContext;
        [SetUp]
        public void Setup()
        {

            var rol = new Rol { idRol = 1, nombreRol = "Rol1test", permisoConfiguracion = true, permisoRol = true, permisoRegistro = true, permisoVisualizacion=true, enabled = true };
            var userTest = new Usuario { idUsuario = 1, email = "adminTest@admin.com", idRol = 1, enabled = true };
            var reporteToEdit = new Reporte { idReporte = 5, nombreReporte = "reporte", link = "link" };
            ...
            _dbContext.Reporte.Add(reporteToEdit);
            _dbContext.SaveChangesAsync();
            controller = new ReportesController(_dbContext);
        ...
        public void TearDown()
        {
            ...
            _dbContext.Reporte.RemoveRange(_dbContext.Reporte);
        ...
        public async Task TestIndexView()
        {
            ...
            Assert.Greater(((IEnumerable<Reporte>)viewResult.Model).Count(), 0);
        }

        [Test]
        public async Task TestVisualizarView()
        {
            var result = await controller.Visualizar(5);
            var viewResult = result as ViewResult;
        ...
        [Test]
        public async Task TestVisualizarAnIdDoesntExistView()
        {
            var result = await controller.Visualizar(99);
        ...
        {
            var reporte = new Reporte { idReporte = 2, nombreReporte = "rep", link="link" };

            var result = await controller.Create(reporte);
        ...
        {
            var reporte = new Reporte { idReporte = 2, nombreReporte = "rep" };
        ...
        {
            _dbContext.Entry(_dbContext.Reporte.Find(5)).State = EntityState.Detached;
            var reporteEdited = new Reporte { idReporte = 5, nombreReporte = "rep", link = "link" };
            var result = await controller.Edit(5, reporteEdited);
