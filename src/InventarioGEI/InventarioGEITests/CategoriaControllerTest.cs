...
namespace InventarioGEITests
{
    public class CategoriaControllerTest
    {
        ...
        public void Setup()
        {
            ...
            var categoriaToEdit = new Categoria { idCategoria = 5, nombreCategoria = "categoriaTest", enabled = true, alcance=alcanceToEdit , usuarios = userTest };
            ...
            _dbContext.Categoria.Add(categoriaToEdit);
            _dbContext.Usuario.Add(userTest);
            _dbContext.SaveChangesAsync();
            controller = new CategoriasController(_dbContext);
        ...
        public async Task TestIndexView()
        {
            ...
            Assert.Greater(((IEnumerable<Categoria>)viewResult.Model).Count(), 0);
        }
        ...
        {
            var categoria = new Categoria { idCategoria = 2, nombreCategoria = "Rol2test", enabled = true };
        ...
        {
            var categoria = new Categoria { idCategoria = 8, enabled = true };
        ...
        {
            _dbContext.Entry(_dbContext.Categoria.Find(5)).State = EntityState.Detached;
            var categoriaEdited = new Categoria { idCategoria = 5, nombreCategoria = "RolEditado", enabled = true };
            var result = await controller.Edit(5, categoriaEdited);
        ...
        {
            var categoria = new Categoria { idCategoria = 5, enabled = true };
            controller.ModelState.AddModelError("nombreAlcance", "Es necesario un nombre para el alcance");

            var result = await controller.Edit(5, categoria);
            var redirectResult = result as RedirectToActionResult;
